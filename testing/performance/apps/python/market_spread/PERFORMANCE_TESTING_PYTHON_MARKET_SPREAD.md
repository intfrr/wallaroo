...
### Visiting the Metrics UI

Using the IP for `wallaroo-leader-1`, you'd visit the following address:

**NOTE:** Replace the IP with the IP output by the `make cluster ...` command

```bash
https://52.3.244.174:4000
```
...
```bash
make cluster cluster_name=<YOUR_CLUSTER_NAME> mem_required=30 cpus_required=36 num_followers=2 force_instance=c4.8xlarge spot_bid_factor=100 ansible_system_cpus=0,18 ansible_isolcpus=true no_spot=true cluster_project_name=wallaroo_perf_testing ansible_install_devtools=true
```
...
```bash
PLAY RECAP *********************************************************************
52.3.244.174               : ok=86   changed=54
52.3.244.175               : ok=86   changed=54
52.3.244.176               : ok=86   changed=54   unreachable=0    failed=0
...
You will have to ssh into each machine to get its hostname, the following hostnames wil be part of this setup:
...
Using the IP for `wallaroo-follower-2`, you'd visit the following address:

**NOTE:** Replace the IP with the IP output by the `make cluster ...` command

```bash
https://52.3.244.176:4000
```
...
```bash
sudo cset proc -s user -e numactl -- -C 16,17 chrt -f 80 ~/wallaroo/giles/receiver/receiver --ponythreads=1 --ponynoblock --ponypinasio -w -l wallaroo-follower-2:5555
```
